...
export const peakUnits = derived(importData, $d => $d.filter(i => i.hour > 17 && i.hour <= 19).map(j => j.kWh))
export const nightUnits = derived(importData, $d => $d.filter(i => i.hour > 23 || i.hour <= 8).map(j => j.kWh))
export const evUnits = derived([importData, evStartTime, evEndTime], ([$data, $start, $end]) => $data.filter(i => i.hour > $start && i.hour <= $end).map(j => j.kWh))
export const dayUnits = derived(importData, $d => $d.filter(i => (i.hour > 8 && i.hour <= 17) || (i.hour > 19 && i.hour <= 23)).map(j => j.kWh))
